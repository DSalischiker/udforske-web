...
const EditSeriesForm = ({ values, handleEditFormClose }) => {
  ...
  const [place, setPlace] = useState(location.name);
  const [loc, setLoc] = useState(location);
  const urlArray = [photos];
          ...
          try {
            ...
            Promise.all(promises)
              .then(async () => {
                ...
                const res = await axios.post("/api/series/edit", {
                  id,
                  title: values.title,
                ...
                setSubmitting(false);
                setMessage(`Serie editada, ${data.title} (${res.status})`);
                handleEditFormClose();
              })
